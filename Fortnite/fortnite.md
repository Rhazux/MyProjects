...
├─assets/
│  ├─css/
│  │    style.css      # vlastní globální styly (sklo, glow, animace, hamburger)
│  └─js/
│       app.js         # veškerá logika na frontendu (JS + AJAX)
│
├─api/
│    weapons.php       # backend endpoint – vrací JSON se zbraněmi
│    contact.php       # backend endpoint – zpracuje kontaktní formulář
│
└─data/
     weapons.json          # seznam zbraní / loadoutů (JSON)
     contact-messages.log  # log zpráv odeslaných z formuláře (generuje se automaticky)
